            ...
            <div class="panel panel-default  "  style="padding:2px;margin-bottom:1px;" >
                ...
                <div class="" style="margin-bottom:1px;padding-top:8px;">
                    ...
                    if (isset($prescribe_items_list)) {
                        echo '<table class="table table-condensed table-hover" style="margin-bottom:0px;">';
                        echo '<tr style="background:#e2e2e2;"><th width="3%">#</th><th width="33%">Name</th><th width="18%">Dose</th><th width="18%">Frequency</th><th width="18%">Period</th><th width="10%">Dose Comment</th>';

                        echo '<th width="5%">Delete</th>';
                        echo '<th width="5%">Print<input type="checkbox" name="print[]" id="select-all" /></th>';
                        ...
                        for ($i = 0; $i < count($prescribe_items_list);  ++$i) {
                            ...
                            echo ($i + 1);
                            echo '</td>';
                            if($prescribe_items_list[$i]["count"]==0){
...
//                            echo $prescribe_items_list[$i]["DRGID"]; //for testing
                            //echo '-';
                            ...
                            echo '</td>';
                            echo '<td>';
                            echo '<select id="final_dosage'.$i. '" onchange=update_dosage('.$prescribe_items_list[$i]["clinic_prescribe_item_id"].','.$i.') style="max-width:40%;">';
                          ...
                          // }
                           ...
                           // echo $prescribe_items_list[$i]["Dosage"];
                            echo '</td>';
                            echo '<td>';
                            echo '<select id="final_frequency'.$i. '" onchange=update_frequency('.$prescribe_items_list[$i]["clinic_prescribe_item_id"].','.$i.') style="max-width:70%;" >';
                            ...
                            echo '</td>';
                            echo '<td>';
                            echo '<select id="final_period'.$i. '"  onchange=update_period('.$prescribe_items_list[$i]["clinic_prescribe_item_id"].','.$i.') style="max-width:50%;">';
...
?>	
		...
		<td><b>Previous prescription<b></td>
            ...
            <tr>
                ...
                <td style="color:red;"><b>Warning Previous prescription<b></td>
            </tr>
            <tr>
                <td width="800px">

                    <table class="" width="800px"  border=0 id="blk_8_content" >
                        <tr id="blk_7_content" >
...
<div class="col-md-2" >
    ...
    <!-- ALLERGY-->
    <div class="panel  panel-danger"  style="padding:2px;margin-bottom:1px;width: 200px" >
...
if ((!isset($patient_allergy_list)) || (empty($patient_allergy_list))) {
    ...
    for ($i = 0; $i < count($patient_allergy_list); ++$i) {
        echo '<tr >';
        echo '<td style="font-weight: bold;font-size: 125%;">';
...
//document.getElementById("blk_favdrug_name_list").disabled = true; 
    ...
    //   2017-09-19 check remaining drugs : Anjana Nisal
   function load_count(id){
       var stock_id = $("#drug_stock_id").val();
       var clinic = <?php echo $clinic_patient_info["clinic"];?>;
       var request = $.ajax({
           url: "<?php echo base_url(); ?>index.php/lookup/get_drug_name",
           type: "post",
           //2017-09-15 clinic only drugs by Akila Madushan Wijesinghe
           data: {"id": id, "drug_stock_id": stock_id, "clinic": clinic}
       });
   ...
   function load_drug_name(id) {
       ...
       var clinic = <?php echo $clinic_patient_info["clinic"];?>;
        ...
        request.done(function (response, textStatus, jqXHR) {
            ...
            //2017-09-19 day of use drugs by Akila Madushan Wijesinghe
            load_warning_previous(<?php echo$clinic_patient_info["clinic"];?>);
...
//                    for clinic default period : 4 weeks -- Anjana Nisal
                    $("#blk_drug_period_list").val('For 4 Weeks');
...
//                     var drug_level = parseInt("+<?php //echo $this->config->item('drug_alert_count'); ?>//");
//2017-09-19 Day of use drug by Akila Madushan Wijesinghe
                    var daysCanUse = data[i]["dayOfUse"];
                    var now = Date();
                    ...
                    if( daysCanUse == 0 || diffDays >= daysCanUse) {
                        //$("#pre_drug_list" + i).append('<td id="pre_drug_name' + i + '" style="color:red;" nowrap></td>');
                   // }else{
                     ...
                     $("#pre_period_list"+ i).append(data[i]["HowLong"]);

//                     $("#pre_drug_list"+ i).append('<td id="pre_date_list' + i + '" nowrap></td>');
//                     $("#pre_date_list"+ i).html('');
//                     $("#pre_date_list"+ i).append(data[i]["CreateDate"]);
...
//2017-09-19 day of use drugs by Akila Madushan Wijesinghe
function load_warning_previous(vid){

    var request = $.ajax({
        url: "<?php echo base_url(); ?>index.php/clinic/get_previous_prescription_for_patient_day_of_use/" + vid + "/" + $("#PID").val(),
        type: "post"
    });
    ...
    request.done(function (response, textStatus, jqXHR) {
        request2.done(function (response2, textStatus, jqXHR) {
            ...
            var data2 = eval('(' + response2 + ')');
            if (!$("#pre_drug7").get(0)) {
                $("#blk_7_content").append('<td id="pre_drug7" width="200px"></td>');
                $("#blk_7_content").append('<td id="pre_dose7" width="100px"></td>');
                $("#blk_7_content").append('<td id="pre_dose_cmnt7" width="150px"></td>');
                $("#blk_7_content").append('<td id="pre_frequency7" width="100px"></td>');
                $("#blk_7_content").append('<td id="pre_period7" width="150px"></td>');
                $("#blk_7_content").append('<td id="pre_days_can_use7" width="150px"></td>');
                $("#blk_7_content").append('<td id="pre_dispensed_days7" width="150px"></td>');
                $("#blk_7_content").append('<td id="pre_continue7" width="200px"></td>');
            }

            var data = eval('(' + response + ')');

            $("#pre_drug7").html('');
            $("#pre_drug7").append('<b>Drug</b>');

            $("#pre_dose7").html('');
            $("#pre_dose7").append('<b>Dose</b>');

            $("#pre_dose_cmnt7").html('');
            $("#pre_dose_cmnt7").append('<b>Comment</b>');

            $("#pre_frequency7").html('');
            $("#pre_frequency7").append('<b>Frequency</b>');

            $("#pre_period7").html('');
            $("#pre_period7").append('<b>Period</b>');

            $("#pre_days_can_use7").html('');
            $("#pre_days_can_use7").append('<b>Days can use</b>');

            $("#pre_dispensed_days7").html('');
            $("#pre_dispensed_days7").append('<b>Dispensed days</b>');

            $("#pre_continue7").html('');
            $("#pre_continue7").append('<b></b>');
        //    $("#pre_continue7").append('<button type="button" style="background-color: #008CBA;font-size: 14px;color: white;" onclick=prescribe_all("' + data[0]["CPRSID"] + '"); >+Repeat All</button>');
            ...
            try {
                for (var i = 0; i < data.length; i++) {
                    ...
                    $("#blk_8_content").append('<tr id="pre_drug_list8' + i + '" nowrap>');
                      var drug_level = parseInt("+<?php echo $this->config->item('drug_alert_count'); ?>");

                    //if (data[i]["who_drug_count"] <= drug_level) {
//                    if(data[i]["name"] == "amitryptyline") {
                    if(data[i]["dayOfUse"] >0 &&  diffDays < daysCanUse) {
                        if (data[i]["dayOfUse"] == 25) {
                            $("#pre_drug_list8" + i).append('<td id="pre_drug_name8' + i + '" style="color:red;" nowrap></td>');
                        } else {
                            $("#pre_drug_list8" + i).append('<td id="pre_drug_name8' + i + '" nowrap></td>');
                        }
                        $("#pre_drug_name8" + i).html('');

                        $("#pre_drug_name8" + i).append(data[i]["name"]);

                        $("#pre_drug_list8" + i).append('<td id="pre_dose_list8' + i + '" nowrap></td>');
                        $("#pre_dose_list8" + i).html('');
                        $("#pre_dose_list8" + i).append(data[i]["Dosage"]);

                        $("#pre_drug_list8" + i).append('<td id="pre_dose_cmnt_list8' + i + '" nowrap></td>');
                        $("#pre_dose_cmnt_list8" + i).html('');
                        $("#pre_dose_cmnt_list8" + i).append(data[i]["DoseComment"]);

                        $("#pre_drug_list8" + i).append('<td id="pre_frequency_list8' + i + '" nowrap></td>');
                        $("#pre_frequency_list8" + i).html('');
                        $("#pre_frequency_list8" + i).append(data[i]["Frequency"]);

                        $("#pre_drug_list8" + i).append('<td id="pre_period_list8' + i + '" nowrap></td>');
                        $("#pre_period_list8" + i).html('');
                        $("#pre_period_list8" + i).append(data[i]["HowLong"]);

                        $("#pre_drug_list8" + i).append('<td id="pre_days_can_use_list8' + i + '" nowrap></td>');
                        $("#pre_days_can_use_list8" + i).html('');
                        $("#pre_days_can_use_list8" + i).append(data[i]["dayOfUse"]);

                        $("#pre_drug_list8" + i).append('<td id="pre_dispensed_days_list8' + i + '" nowrap></td>');
                        $("#pre_dispensed_days_list8" + i).html('');
                        $("#pre_dispensed_days_list8" + i).append(diffDays);

                        $("#pre_drug_list8" + i).append('<td id="pre_continue_list8' + i + '" nowrap></td>');
                        $("#pre_continue_list8" + i).html('');
                        $("#pre_continue_list8" + i).append('<td nowrap><span class="continue"  onmouseover="drugSelect(' + data[i]["wd_id"] + ',' + data[i]["Dosage"] + ',\'' + data[i]["DoseComment"] + '\',\'' + data[i]["Frequency"] + '\',\'' + data[i]["HowLong"] + '\')"   onclick=$("#drug_form").submit(); >Continue this</span></td>');

                        $("#pre_drug_list8" + i).append('<td id="pre_period_list8' + i + '" nowrap></td>');
                        $("#pre_period_list8" + i).html('');
                        $("#pre_period_list8" + i).append(data[i]["HowLong"]);

                        $("#pre_days_can_use_list8" + i).append('<td id="pre_days_can_use_list8' + i + '" nowrap></td>');
                        $("#pre_days_can_use_list8" + i).html('');
                        $("#pre_days_can_use_list8" + i).append(data[i]["dayOfUse"]);

                    $("#blk_8_content").append('</tr>');
                    // PRES_ID = data[i]["PRES_ID"]
                    }
       ...
       var request = $.ajax({
            url: "<?php echo base_url(); ?>index.php/clinic/update_dosage/",
            type: "post",
             data: {
    ...
    request.done(function (response, textStatus, jqXHR) {
       ...
       var request = $.ajax({
            url: "<?php echo base_url(); ?>index.php/clinic/update_frequency/" + prs_item_id + "/" + new_frequency,
            type: "post"
        });
    ...
    request.done(function (response, textStatus, jqXHR) {
       ...
       var new_period=$("#final_period"+id).val();
        var request = $.ajax({
            url: "<?php echo base_url(); ?>index.php/clinic/update_period/" + prs_item_id + "/" + new_period,
            type: "post"
        });
    ...
    function prescribe_all_favour(favour_id, count) {
       ...
       var prsid=$("#clinic_prescription_id").val();
     if(prsid=='') {
       prsid=0;
   }

            var request = $.ajax({
                url: "<?php echo base_url(); ?>index.php/clinic/prescribe_all_favour/" + favour_id + "/" + $("#PID").val() + "/" + $("#clinic_patient_id").val()+"/"+prsid,
                type: "post"
            });
            request.done(function (response, textStatus, jqXHR) {
              if (response > 0) {
    ...
    function send_to_pharmacy(id, clnid) {
        ...
        // Printing OS drugs when pressing send to pharmacy : Anjana Nisal
        var dat,datResponse;
        var items_array=[];
        <?php
        if(isset($prescribe_items_list) && count($prescribe_items_list)>0){ ?>
        var datCount = '<?php echo count($prescribe_items_list);?>'
            datResponse = '<?php echo json_encode($prescribe_items_list);?>'
         dat = JSON.parse(datResponse);
        <?php } ?>
        for(var x=0 ; x < parseInt(datCount) ; x++){
            if(dat[x]['count'] <= 0){
                data += 'print[]=' + dat[x]['clinic_prescribe_item_id']+'&';
                items_array.push(dat[x]['clinic_prescribe_item_id']);

            }
        }
        var pId= <?php echo json_encode($clinic_patient_info['PID']);?>;

        data += 'pid='+pId;
        var params = 'menubar=no,location=no,resizable=yes,scrollbars=yes,status=no,width=750,height=700';
        if(items_array.length>0){
            window.open("<?php echo base_url(); ?>" + "index.php/report/pdf/ClinicoutsidePrescription/auto_print/"+data, 'lookUpW',params);
        }
    ...
    function check_alergy_favlist(favour_id){
    	...
    	if (favour_id == "")
            return;
        var request = $.ajax({
        url: "<?php echo base_url(); ?>index.php/clinic/check_alergy_favlist/" + favour_id,
        type: "post"
